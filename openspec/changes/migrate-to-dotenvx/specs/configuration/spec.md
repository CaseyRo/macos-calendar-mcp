...
The system SHALL support loading configuration from a `.env` file in the project root directory, in addition to environment variables and default values, using `@dotenvx/dotenvx` for enhanced security and cross-platform consistency.

#### Scenario: Load configuration from .env file
- **WHEN** a `.env` file exists in the project root with configuration values
- **AND** the server is started
- **THEN** the server loads configuration from the `.env` file using `dotenvx`
...
The system SHALL include clear documentation explaining how to use `.env` files for configuration, including reference to `dotenvx` and optional encryption capabilities.
...
- **AND** documentation references `dotenvx` as the underlying library
